...
export default function ProfileView({ bookings = [], onRemove, onEdit }: { bookings?: Booking[], onRemove?: (id: string) => void, onEdit?: (b: Booking) => void }) {
  const now = new Date();
        ...
        <div className="flex flex-col sm:flex-row justify-between md:items-center gap-2 mb-3">
           ...
           {!isPast && (
             <div className="flex items-center gap-2 flex-shrink-0">
               ...
               <button onClick={() => onRemove?.(b.id)} className="text-xs font-bold text-error bg-error/10 px-3 py-1.5 rounded-lg hover:bg-error/20 transition-colors">Cancel</button>
             </div>
           )}
        </div>
  ...
  return (
    <motion.section 
      ...
      className="px-4 md:px-6 py-6 flex-1 flex flex-col pt-12 md:pt-6 pb-24 md:pb-6 max-w-2xl mx-auto w-full overflow-hidden"
    >
      <div className="mb-10">
